...
def setup_spark(master_nodes,slave_nodes):
	...
	ssh_command(master,"cd december;cd spark-1.2.0-bin-cdh4/conf;echo 'export JAVA_HOME=/usr/lib/jvm/java-1.7.0-openjdk-1.7.0.75.x86_64' >> spark-env.sh")
...
def setup_shark(master_nodes,slave_nodes):
	...
	ssh_command(master,"cd december;wget https://s3.amazonaws.com/spark-ui/hive-0.11.0-bin.tgz")
	ssh_command(master,"cd december;wget https://s3.amazonaws.com/spark-ui/shark-0.9-hadoop-2.0.0-mr1-cdh4.2.0.tar.gz")
	ssh_command(master,"cd december;tar zxf hive-0.11.0-bin.tgz")
	ssh_command(master,"cd december;tar zxf shark-0.9-hadoop-2.0.0-mr1-cdh4.2.0.tar.gz")
	ssh_command(master,"rm december/hive-0.11.0-bin.tgz")
	ssh_command(master,"rm december/shark-0.9-hadoop-2.0.0-mr1-cdh4.2.0.tar.gz")

	print '[ Configuring Shark ]'
	ssh_command(master,"cd december/shark/;echo \"export SHARK_MASTER_MEM=1g\" > conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"SPARK_JAVA_OPTS+=\\\" -Dspark.kryoserializer.buffer.mb=10 \\\"\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export SPARK_JAVA_OPTS\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export HIVE_HOME=/home/`whoami`/december/hive-0.11.0-bin\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export SPARK_JAVA_OPTS\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export MASTER=spark://PUT_MASTER_IP_HERE:7077\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export SPARK_HOME=/home/`whoami`/december/spark-0.9.1-bin-cdh4\" >> conf/shark-env.sh")
	ssh_command(master,"mkdir /mnt/tachyon")
	ssh_command(master,"cd december/shark/;echo \"export TACHYON_MASTER=PUT_MASTER_IP_HERE:19998\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"export TACHYON_WAREHOUSE_PATH=/mnt/tachyon\" >> conf/shark-env.sh")
	ssh_command(master,"cd december/shark/;echo \"source /home/`whoami`/december/spark-0.9.1-bin-cdh4/conf/spark-env.sh\" >> conf/shark-env.sh")
	...
	ssh_command(master,"chmod +x december/shark/conf/shark-env.sh")
	...
	ssh_command(master,"cd december/shark/;./bin/shark --service sharkserver 10000 > log.txt 2>&1 &")
